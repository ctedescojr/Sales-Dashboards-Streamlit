...
def calcular_curva_abc(df, metrica='Valor'):
    ...
    produto_metrica['Classificacao'] = produto_metrica['Porcentagem_Acumulada'].apply(classificar)
...
def produtos_comprados_juntos(df):
    # Agrupar por pedido
    pedidos_produtos = df.groupby(['Pedido', 'Produto'])['Quantidade'].sum().reset_index()
    ...
    for pedido in pedidos_produtos['Pedido'].unique():
        ...
        produtos = pedidos_produtos[pedidos_produtos['Pedido'] == pedido]['Produto'].tolist()
    ...
    pares_df = pd.DataFrame([(p1, p2, count) for (p1, p2), count in pares_produtos.items()],
...
def perfil_consumidor(df):
    ...
    cliente_analise['Valor_Medio_Pedido'] = cliente_analise['Valor'] / cliente_analise['Num_Pedidos']
...
try:
    ...
    if df.empty:
        ...
        mes_selecionado = st.sidebar.selectbox("Selecione o Mês", ["Todos"] + list(meses))
        ...
        with col1:
            ...
            st.metric("Total de Vendas", f"R$ {valor_total:,.2f}".replace(',', '.').replace('.', ',', 1))
        
        with col2:
            st.metric("Número de Pedidos", f"{df_filtrado['Pedido'].nunique():,}".replace(',', '.'))
        
        with col3:
            st.metric("Número de Clientes", f"{df_filtrado['Cliente'].nunique():,}".replace(',', '.'))
        
        with col4:
            ticket_medio = df_filtrado['Valor'].sum() / df_filtrado['Pedido'].nunique() if df_filtrado['Pedido'].nunique() > 0 else 0
            st.metric("Ticket Médio", f"R$ {ticket_medio:,.2f}".replace(',', '.').replace('.', ',', 1))
        ...
        with col1:
            metrica_abc = st.radio("Métrica para Curva ABC", ["Valor", "Quantidade"], horizontal=True)
        with col2:
            filtro_abc = st.multiselect("Filtrar por Classificação", ["A", "B", "C"], default=["A", "B", "C"])
        ...
        for classificacao in curva_abc_filtrada['Classificacao'].unique():
            df_class = curva_abc_filtrada[curva_abc_filtrada['Classificacao'] == classificacao]
        ...
        if set(filtro_abc) == set(['A', 'B', 'C']):
            ...
            curva_abc_filtrada = curva_abc_filtrada.sort_values(metrica_abc, ascending=False).copy()
            curva_abc_filtrada['Porcentagem_Filtrada'] = curva_abc_filtrada[metrica_abc] / total_filtrado * 100
            curva_abc_filtrada['Porcentagem_Acumulada_Filtrada'] = curva_abc_filtrada['Porcentagem_Filtrada'].cumsum()
        ...
        if 'Valor' in curva_abc_display.columns:
            curva_abc_display['Valor'] = curva_abc_display['Valor'].apply(lambda x: f"R$ {x:,.2f}".replace(',', '.').replace('.', ',', 1))
        if 'Quantidade' in curva_abc_display.columns:
            curva_abc_display['Quantidade'] = curva_abc_display['Quantidade'].apply(lambda x: f"{x:,.0f}".replace(',', '.'))
        curva_abc_display['Porcentagem'] = curva_abc_display['Porcentagem'].apply(lambda x: f"{x:.2f}%".replace('.', ','))
        curva_abc_display['Porcentagem_Acumulada'] = curva_abc_display['Porcentagem_Acumulada'].apply(lambda x: f"{x:.2f}%".replace('.', ','))
        ...
        with col1:
            ...
            st.metric("Produtos A", len(produtos_a), f"{len(produtos_a)/len(curva_abc)*100:.1f}%".replace('.', ',') + " do total")
            
            if metrica_abc == 'Valor':
                valor_a = produtos_a['Valor'].sum()
                valor_total = curva_abc['Valor'].sum()
                st.metric(f"{metrica_abc} Produtos A", 
        ...
        with col2:
            ...
            st.metric("Produtos B", len(produtos_b), f"{len(produtos_b)/len(curva_abc)*100:.1f}%".replace('.', ',') + " do total")
            
            if metrica_abc == 'Valor':
                valor_b = produtos_b['Valor'].sum()
                valor_total = curva_abc['Valor'].sum()
                st.metric(f"{metrica_abc} Produtos B", 
        ...
        with col3:
            ...
            st.metric("Produtos C", len(produtos_c), f"{len(produtos_c)/len(curva_abc)*100:.1f}%".replace('.', ',') + " do total")
            
            if metrica_abc == 'Valor':
                valor_c = produtos_c['Valor'].sum()
                valor_total = curva_abc['Valor'].sum()
                st.metric(f"{metrica_abc} Produtos C", 
        ...
        if not pares_produtos.empty:
            ...
            fig_pares = px.bar(
                pares_produtos.head(10),
                x='Frequencia',
                y=pares_produtos.head(10).apply(lambda x: f"{x['Produto1']} + {x['Produto2']}", axis=1),
                orientation='h',
            ...
            st.info("Não há dados suficientes para análise de produtos comprados juntos.")
        ...
        perfil_display['Valor'] = perfil_display['Valor'].apply(lambda x: f"R$ {x:,.2f}".replace(',', '.').replace('.', ',', 1))
        perfil_display['Valor_Medio_Pedido'] = perfil_display['Valor_Medio_Pedido'].apply(lambda x: f"R$ {x:,.2f}".replace(',', '.').replace('.', ',', 1))
        perfil_display['Num_Pedidos'] = perfil_display['Num_Pedidos'].apply(lambda x: f"{x:,}".replace(',', '.'))
        ...
        fig_dispersao = px.scatter(
            ...
            labels={'Num_Pedidos': 'Número de Pedidos', 'Valor': 'Valor Total (R$)', 'Valor_Medio_Pedido': 'Valor Médio por Pedido (R$)'},
        ...
        # Vendas por mês
        vendas_mes = df.groupby(['Ano', 'Mes', 'Mes_Nome'])['Valor'].sum().reset_index()
        ...
        fig_mes = px.bar(
            ...
            labels={'Mes_Nome': 'Mês', 'Valor': 'Valor Total (R$)', 'Ano': 'Ano'},
            barmode='group'
        )
